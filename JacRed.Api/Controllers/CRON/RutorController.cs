...
public class RutorController : BaseController
{
    ...
    public async Task<string> UpdateTasksParse()
    {
        ...
        {
            var html = await _httpService.Get($"{AppInit.conf.Rutor.rqHost()}/browse/0/{cat}/0/0",
    ...
    {
        var html = await _httpService.Get($"{AppInit.conf.Rutor.rqHost()}/browse/{page}/{cat}/0/0",
            useProxy: AppInit.conf.Rutor.useproxy);
                     ...
                     .Skip(1))
        {
            ...
            if (cat == "17" && !title.Contains(" UKR")) continue;

            if (title.Contains(" КПК")) continue;
    ...
    public RutorController(IMemoryCache memoryCache, ITorrentRepository torrentRepository, HttpService httpService) :
        base(memoryCache)
    {
