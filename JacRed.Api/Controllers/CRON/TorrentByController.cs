...
public class TorrentByController : BaseController
{
	...
	public async Task<string> UpdateTasksParse()
	{
		...
		{
			// Получаем html
			var html = await HttpClient.Get($"{AppInit.conf.TorrentBy.rqHost()}/{cat}/", timeoutSeconds: 10,
	...
	{
		var html = await HttpClient.Get($"{AppInit.conf.TorrentBy.rqHost()}/{cat}/?page={page}",
			useproxy: AppInit.conf.TorrentBy.useproxy);
	...
	public TorrentByController(IMemoryCache memoryCache, ITorrentRepository torrentRepository) : base(memoryCache)
	{
