...
public class TorrentByController : BaseController
{
    ...
    static TorrentByController()
    {
        if (IO.File.Exists("Data/temp/torrentby_taskParse.json"))
            taskParse = JsonConvert.DeserializeObject<Dictionary<string, List<TaskParse>>>(
                IO.File.ReadAllText("Data/temp/torrentby_taskParse.json"));
    }
    ...
    public async Task<string> UpdateTasksParse()
    {
        ...
        {
            // Получаем html
            var html = await _httpService.Get($"{AppInit.conf.TorrentBy.rqHost()}/{cat}/", timeoutSeconds: 10,
                useProxy: AppInit.conf.TorrentBy.useproxy);

            if (html == null) continue;

            // Максимальное количиство страниц
            int.TryParse(Regex.Match(html, "href=\"\\?page=([0-9]+)\">[0-9]+</a>([\t ]+)?</center></td>")
                    .Groups[1].Value,
                out var maxpages);
    ...
    {
        var html = await _httpService.Get($"{AppInit.conf.TorrentBy.rqHost()}/{cat}/?page={page}",
            useProxy: AppInit.conf.TorrentBy.useproxy);
                     ...
                     .Skip(1))
        {
            ...
            if (string.IsNullOrWhiteSpace(row) || !row.Contains("magnet:?xt=urn")) continue;
            ...
            {
                var _createTime = Match(">([0-9]{4}-[0-9]{2}-[0-9]{2})</td>")
                    .Replace("-", " ");

                if (!DateTime.TryParseExact(_createTime, "yyyy MM dd", new CultureInfo("ru-RU"), DateTimeStyles.None,
                        out createTime))
                    continue;
            }
            ...
            var url = Match("<a name=\"search_select\" [^>]+ href=\"/([0-9]+/[^\"]+)\"");
            var title = Match("<a name=\"search_select\" [^>]+>([^<]+)</a>");
            var _sid = Match("<font color=\"green\">&uarr; ([0-9]+)</font>");
            var _pir = Match("<font color=\"red\">&darr; ([0-9]+)</font>");
            var sizeName = Match("</td><td style=\"white-space:nowrap;\">([^<]+)</td>");
    ...
    public TorrentByController(IMemoryCache memoryCache, ITorrentRepository torrentRepository, HttpService httpService)
        : base(memoryCache)
    {
