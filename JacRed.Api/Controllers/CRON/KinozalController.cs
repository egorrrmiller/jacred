...
public class KinozalController : BaseController
{
	...
	public async Task<string> UpdateTasksParse()
	{
				...
				})
		{
			...
			{
				// Получаем html
				var html = await HttpClient.Get($"{AppInit.conf.Kinozal.host}/browse.php?c={cat}&d={year}&t=1",
	...
	{
		var html = await HttpClient.Get($"{AppInit.conf.Kinozal.host}/browse.php?c={cat}&page={page}" + arg,
	...
	public KinozalController(IMemoryCache memoryCache, ITorrentRepository torrentRepository) : base(memoryCache)
	{
