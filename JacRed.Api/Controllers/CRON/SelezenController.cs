...
public class SelezenController : BaseController
{
	...
	{
		// Получаем html
		var html = await HttpClient.Get($"{AppInit.conf.Selezen.host}/relizy-ot-selezen/",
			timeoutSeconds: 10,
	...
	private async Task<bool> parsePage(int page)
	{
		...
		var html = await HttpClient.Get(page == 1
	...
	public SelezenController(IMemoryCache memoryCache, ITorrentRepository torrentRepository) : base(memoryCache)
	{
