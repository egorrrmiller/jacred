...
public class LostfilmController : BaseController
{
	...
	{
		var html = await Core.Utils.HttpClient.Get(page > 1
			...
			try
			{
				...
				} else
				{
					...
					if (series != null)
					{
						...
						if (int.TryParse(dateCreated, out var _date) && _date > 0)
						{
							relased = _date;
						}
					}
	...
	public LostfilmController(IMemoryCache memoryCache, ITorrentRepository torrentRepository) : base(memoryCache)
	{
			...
			if (!string.IsNullOrWhiteSpace(fullNews))
			{
				...
				if (!string.IsNullOrWhiteSpace(episodeId))
				{
					...
					if (!string.IsNullOrWhiteSpace(retreSearchUrl))
					{
						...
						{
							//Console.WriteLine("7");
							var match = new Regex("<div class=\"inner-box--link main\"><a href=\"([^\"]+)\">([^<]+)</a></div>")
							...
							while (match.Success)
							{
										...
										RegexOptions.IgnoreCase))
								{
									...
									if (!string.IsNullOrWhiteSpace(torrentFile) && !string.IsNullOrWhiteSpace(quality))
									{
										...
										var torrent = await Core.Utils.HttpClient.Download(torrentFile,
