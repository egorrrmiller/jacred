...
public class AnimeLayerController : BaseController
{
	...
	{
		var html = await HttpClient.Get($"{AppInit.conf.Animelayer.host}/torrents/anime/?page={page}",
					...
					.Skip(1))
		{
			...
			// Год выхода
			if (!int.TryParse(Match("Год выхода: ?</strong>([0-9]{4})"), out var relased) || relased == 0)
			{
				continue;
			}
	...
	public AnimeLayerController(IMemoryCache memoryCache, ITorrentRepository torrentRepository) : base(memoryCache) =>
