...
public class MegapeerController : BaseController
{
    ...
    public async Task<string> UpdateTasksParse()
    {
        ...
        {
            var html = await _httpService.Get($"{AppInit.conf.Megapeer.rqHost()}/browse.php?cat={cat}",
    ...
    {
        var html = await _httpService.Get($"{AppInit.conf.Megapeer.rqHost()}/browse.php?cat={cat}&page={page}",
            Encoding.GetEncoding(1251), useProxy: AppInit.conf.Megapeer.useproxy,
            addHeaders: new List<(string name, string val)>
            {
                ("dnt", "1"),
        ...
        await _torrentRepository.AddOrUpdateAsync(torrents, async (t, db) =>
        {
            ...
            var _t = await _httpService.Download($"{AppInit.conf.Megapeer.host}/download/{t.downloadId}",
    ...
    public MegapeerController(IMemoryCache memoryCache, ITorrentRepository torrentRepository, HttpService httpService) :
        base(memoryCache)
    {
