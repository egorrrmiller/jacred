...
public class BitruController : BaseController
{
	...
	public async Task<string> UpdateTasksParse()
	{
		...
		{
			// Получаем html
			var html = await HttpClient.Get($"{AppInit.conf.Bitru.rqHost()}/browse.php?tmp={cat}", timeoutSeconds: 10,
	...
	{
		var html = await HttpClient.Get($"{AppInit.conf.Bitru.rqHost()}/browse.php?tmp={cat}&page={page}",
			useproxy: AppInit.conf.Bitru.useproxy);
