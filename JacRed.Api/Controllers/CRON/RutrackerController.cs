...
public class RutrackerController : BaseController
{
    ...
    private static readonly HashSet<string> firstPageCats = new()
    {
        // 3D Кинофильмы
        "549",
    ...
    static RutrackerController()
    {
        if (IO.File.Exists("Data/temp/rutracker_taskParse.json"))
            taskParse = JsonConvert.DeserializeObject<Dictionary<string, List<TaskParse>>>(
                IO.File.ReadAllText("Data/temp/rutracker_taskParse.json"));
    }
    ...
    {
        foreach (var cat in new HashSet<string>(firstPageCats)
                 {
            ...
            {
                // Получаем html
                var html = await _httpService.Get($"{AppInit.conf.Rutracker.rqHost()}/forum/viewforum.php?f={cat}",
                    useProxy: AppInit.conf.Rutracker.useproxy);

                if (html == null) continue;

                // Максимальное количиство страниц
                int.TryParse(Regex.Match(html, "Страница <b>1</b> из <b>([0-9]+)</b>")
                        .Groups[1].Value,
                    out var maxpages);
                ...
                else
                {
                    ...
                    if (val.FirstOrDefault(i => i.page == 1) == null) val.Add(new TaskParse(1));
                }
            }
            catch
            {
    ...
    private async Task<bool> parsePage(string cat, int page)
    {
        ...
        var html = await _httpService.Get(
            $"{AppInit.conf.Rutracker.rqHost()}/forum/viewforum.php?f={cat}{(page == 0 ? "" : $"&start={page * 50}")}", /*cookie: Cookie, */
            useProxy: AppInit.conf.Rutracker.useproxy);

        if (html == null /*|| !html.Contains("id=\"logged-in-username\"")*/) return false;
                     ...
                     .Skip(1))
        {
            ...
            DateTime.TryParse(Match("<p>([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2})</p>"), out var createTime);
            ...
            var url = Match("<a id=\"tt-([0-9]+)\"");
            var title = Match("<a id=\"tt-[0-9]+\"[^>]+>([^\n\r]+)</a>");
            title = Regex.Replace(title, "<[^>]+>", "");
            var _sid = Match("<span class=\"seedmed\"[^>]+><b>([0-9]+)</b>");
            var _pir = Match("<span class=\"leechmed\"[^>]+><b>([0-9]+)</b>");

            var sizeName = Match("dl-stub\">([^<]+)</a>")
                .Replace("&nbsp;", " ");
            ...
            if (cat is "549" or "1213" or "2109" or "22" or "1666" or "941" or "1950" or "1950" or "2090" or "2221"
                or "2091" or "2092" or "2093" or "2200" or "2540" or "934" or "505" or "124" or "1457"
                or "2199" or "313" or "312" or "1247" or "2201" or "2339" or "140" or "2343" or "930" or "2365" or "208"
                or "539" or "209" or "709" or "252")
            {
                #region Фильмы
                ...
                if (name != null)
                    name = name.Replace("в 3Д", "")
                        .Trim();

                if (originalname != null)
                    originalname = originalname.Replace(" in 3D", "")
                        .Replace(" 3D", "")
                        .Trim();
            }
            else if (cat is "842" or "235" or "242" or "819" or "1531" or "721" or "1102" or "1120" or "1214" or "489"
                     or "387" or "9" or "81" or "119" or "1803" or "266" or "193" or "1690" or "1459" or "825" or "1248"
                     or "1288"
                     or "325" or "534" or "694" or "704" or "921" or "815" or "1460")
            {
                ...
                if (Regex.IsMatch(title, "(Сезон|Серии)", RegexOptions.IgnoreCase))
                {
                    ...
                    if (Regex.IsMatch(name ?? "", "(Сезон|Серии)", RegexOptions.IgnoreCase)
                        || Regex.IsMatch(originalname ?? "", "(Сезон|Серии)", RegexOptions.IgnoreCase))
                    {
                        relased = 0;
                        name = null;
                        originalname = null;
            ...
            else if (cat is "1105" or "2491" or "1389" or "915" or "1939" or "46" or "671" or "2177" or "2538" or "251"
                     or "98" or "97" or "851" or "2178" or "821" or "2076" or "56" or "2123" or "876" or "2139"
                     or "1467"
                     or "1469" or "249" or "552" or "500" or "2112" or "1327" or "1468" or "2168" or "2160" or "314"
                     or "1281" or "2110" or "979" or "2169" or "2164" or "2166" or "2163"
                     or "24" or "1959" or "939" or "1481" or "113" or "115" or "882" or "1482" or "393" or "2537"
                     or "532" or "827")
            {
                #region Нестандартные титлы

                name = Regex.Match(title, "^([^/\\(\\[]+) ")
                    .Groups[1].Value;

                if (int.TryParse(Regex.Match(title, " \\[([0-9]{4})(,|-) ")
                        .Groups[1].Value, out var _yer))
                    relased = _yer;

                if (Regex.IsMatch(name ?? "", "(Сезон|Серии)", RegexOptions.IgnoreCase)) continue;
        ...
        await _torrentRepository.AddOrUpdateAsync(torrents, async (t, db) =>
        {
            ...
            var fullNews = await _httpService.Get(t.Url, useProxy: AppInit.conf.Rutracker.useproxy);
            ...
            if (fullNews != null)
            {
                ...
                if (createTime != default) t.CreateTime = createTime;
    ...
    public RutrackerController(IMemoryCache memoryCache, ITorrentRepository torrentRepository, HttpService httpService)
        : base(memoryCache)
    {
    ...
    private async ValueTask<bool> TakeLogin()
    {
        var authKey = "rutracker:TakeLogin()";
        ...
        try
        {
            ...
            using (var client = new HttpClient(clientHandler))
            {
                ...
                {
                    using (var response =
                           await client.PostAsync($"{AppInit.conf.Rutracker.host}/forum/login.php", postContent))
                    {
                        if (response.Headers.TryGetValues("Set-Cookie", out var cook))
                        {
                            ...
                            foreach (var line in cook)
                            {
                                ...
                                if (line.Contains("bb_session="))
                                    session = new Regex("bb_session=([^;]+)(;|$)").Match(line)
                                        .Groups[1].Value;
                            }
    ...
    public async Task<string> Parse(int page)
    {
        ...
        try
        {
            foreach (var cat in firstPageCats)
            {
                var result = await parsePage(cat, page);
                log += $"{cat} - {page} - {result}\n";
            }
        }
        catch
