...
public class RutrackerController : BaseController
{
	...
	public async Task<string> UpdateTasksParse()
	{
				...
				})
		{
			...
			{
				// Получаем html
				var html = await HttpClient.Get($"{AppInit.conf.Rutracker.rqHost()}/forum/viewforum.php?f={cat}",
	...
	public RutrackerController(IMemoryCache memoryCache, ITorrentRepository torrentRepository) : base(memoryCache)
	{
	...
	private async ValueTask<bool> TakeLogin()
	{
		...
		if (MemoryCache.TryGetValue(authKey, out var _))
		{
			return false;
		}
