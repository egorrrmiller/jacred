...
public class BaibakoController : BaseController
{
    ...
    {
        var html = await _httpService.Get($"{AppInit.conf.Baibako.host}/browse.php?page={page}",
            Encoding.GetEncoding(1251), Cookie(MemoryCache));

        if (html == null || !html.Contains("id=\"navtop\"")) return false;
                     ...
                     .Skip(1))
        {
            ...
            // Дата создания
            var createTime = MediaNameUtils.ParseDate(
                Match("<small>Загружена: ([0-9]+ [^ ]+ [0-9]{4}) в [^<]+</small>"),
                "dd.MM.yyyy");
            ...
            if (string.IsNullOrWhiteSpace(url) || string.IsNullOrWhiteSpace(title) ||
                !Regex.IsMatch(title, "(1080p|720p)")) continue;
        ...
        await _torrentRepository.AddOrUpdateAsync(torrents, async (t, db) =>
        {
            ...
            var torrent = await _httpService.Download(t.downloadUri, Cookie(MemoryCache),
    ...
    public BaibakoController(IMemoryCache memoryCache, ITorrentRepository torrentRepository, HttpService httpService) :
        base(memoryCache)
    {
    ...
    {
        if (memoryCache.TryGetValue("baibako:cookie", out string cookie)) return cookie;
                ...
                using (var postContent = new FormUrlEncodedContent(postParams))
                {
                           ...
                           await client.PostAsync($"{AppInit.conf.Baibako.host}/takelogin.php", postContent))
                    {
                        if (response.Headers.TryGetValues("Set-Cookie", out var cook))
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(sessid) && !string.IsNullOrWhiteSpace(uid) &&
    ...
    public async Task<string> Parse(int maxpage)
    {
        ...
        try
        {
            ...
            {
                if (page > 1) await Task.Delay(AppInit.conf.Baibako.parseDelay);
