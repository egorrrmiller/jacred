...
public class AnifilmController : BaseController
{
    ...
    {
        var html = await _httpService.Get($"{AppInit.conf.Anifilm.rqHost()}/releases/page/{page}?category={cat}",
            useProxy: AppInit.conf.Anifilm.useproxy);

        if (html == null || !html.Contains("id=\"ui-components\"")) return false;
                     ...
                     .Skip(1))
        {
            ...
            if (string.IsNullOrWhiteSpace(url) || string.IsNullOrWhiteSpace(name) ||
                string.IsNullOrWhiteSpace(originalname)) continue;

            if (cat != "movies" && string.IsNullOrWhiteSpace(episodes)) continue;

            url = $"{AppInit.conf.Anifilm.host}/{url}";
            var title = $"{name} / {originalname}";

            if (!string.IsNullOrWhiteSpace(episodes)) title += $" ({episodes})";
            ...
            // Год выхода
            if (!int.TryParse(Match("<a href=\"/releases/releases/[^\"]+\">([0-9]{4})</a> г\\."), out var relased) ||
                relased == 0) continue;
        ...
        {
            if (db.TryGetValue(t.Url, out var _tcache) && _tcache.Title.Replace(" [1080p]", "") == t.Title) return true;

            var fullNews = await _httpService.Get(t.Url, useProxy: AppInit.conf.Anifilm.useproxy);
            ...
            if (fullNews != null)
            {
                ...
                if (!string.IsNullOrWhiteSpace(_rnews))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(tid) && !title.Contains(" [1080p]")) title += " [1080p]";
                }
                ...
                if (!string.IsNullOrWhiteSpace(tid))
                {
                    var torrent = await _httpService.Download($"{AppInit.conf.Anifilm.host}/{tid}", referer: t.Url,
                        useProxy: AppInit.conf.Anifilm.useproxy);
    ...
    public AnifilmController(IMemoryCache memoryCache, ITorrentRepository torrentRepository, HttpService httpService) :
        base(memoryCache)
    {
