...
public class HDRezkaController : BaseController
{
	...
	{
		var html = await HttpClient.Get(AppInit.conf.Rezka.rqHost()
										+ (page > 1
											? $"/page/{page}"
											: ""),
					...
					.Reverse())
		{
			...
			if (!string.IsNullOrWhiteSpace(url))
			{
				...
				if (MemoryCache.TryGetValue(url, out var _))
				{
					continue;
				}

				var fulnews = await HttpClient.Get(url, useproxy: AppInit.conf.Rezka.useproxy);
				...
				if (!int.TryParse(g[2].Value, out var relased) || relased == 0)
				{
					continue;
				}
						...
						})
				{
					...
					torrent = await HttpClient.Download($"{AppInit.conf.Rezka.rqHost()}/{tid}", referer: url,
						useproxy: AppInit.conf.Rezka.useproxy);
	...
	public HDRezkaController(IMemoryCache memoryCache, ITorrentRepository torrentRepository) : base(memoryCache)
	{
