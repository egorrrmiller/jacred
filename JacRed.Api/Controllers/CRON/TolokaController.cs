...
public class TolokaController : BaseController
{
    ...
    static TolokaController()
    {
        if (IO.File.Exists("Data/temp/toloka_taskParse.json"))
            taskParse = JsonConvert.DeserializeObject<Dictionary<string, List<TaskParse>>>(
                IO.File.ReadAllText("Data/temp/toloka_taskParse.json"));
    }
    ...
    public async Task<string> UpdateTasksParse()
    {
        ...
        if (Cookie(MemoryCache) == null)
        {
            ...
            if (!await TakeLogin(MemoryCache))
            {
        ...
        {
            // Получаем html
            var html = await _httpService.Get($"{AppInit.conf.Toloka.host}/f{cat}", timeoutSeconds: 10,
                cookie: Cookie(MemoryCache));

            if (html == null) continue;

            // Максимальное количиство страниц
            int.TryParse(Regex.Match(html, ">([0-9]+)</a>&nbsp;&nbsp;<a href=\"[^\"]+\">наступна</a>")
                    .Groups[1].Value,
                out var maxpages);
    ...
    private async Task<bool> parsePage(string cat, int page)
    {
        ...
        var html = await _httpService.Get(
            $"{AppInit.conf.Toloka.host}/f{cat}{(page == 0 ? "" : $"-{page * 45}")}?sort=8",
            cookie: Cookie(MemoryCache) /*, useproxy: true, proxy: tParse.webProxy()*/);

        if (html == null || !html.Contains("<html lang=\"uk\"")) return false;
        ...
        foreach (var row in MediaNameUtils.Normalize(html)
                     ...
                     .Skip(1))
        {
            if (string.IsNullOrWhiteSpace(row) || Regex.IsMatch(row, "Збір коштів", RegexOptions.IgnoreCase)) continue;
            ...
            var _createTime = Match("class=\"postdetails\">([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2})")
                .Replace("-", ".");

            if (!DateTime.TryParse(_createTime, out var createTime) || createTime == default) continue;

            #endregion

            #region Данные раздачи

            var url = Match("<a href=\"(t[0-9]+)\" class=\"topictitle\"");
            var title = Match("class=\"topictitle\">([^<]+)</a>");

            var _sid = Match("<span class=\"seedmed\" [^>]+><b>([0-9]+)</b></span>");
            var _pir = Match("<span class=\"leechmed\" [^>]+><b>([0-9]+)</b></span>");

            var sizeName = Match("<a href=\"download.php[^\"]+\" [^>]+>([^<]+)</a>")
                .Replace("&nbsp;", " ");
            ...
            else if (cat is "32" or "173" or "174" or "44" or "230" or "226" or "227" or "228" or "229" or "127"
                     or "124" or "125" or "132")
            {
                #region Сериалы
            ...
            if (!string.IsNullOrWhiteSpace(name))
            {
                ...
                var downloadId = Regex.Match(row, "href=\"download.php\\?id=([0-9]+)\"")
                    .Groups[1].Value;

                if (string.IsNullOrWhiteSpace(downloadId)) continue;
        ...
        await _torrentRepository.AddOrUpdateAsync(torrents, async (t, db) =>
        {
            ...
            var torrent = await _httpService.Download($"{AppInit.conf.Toloka.host}/download.php?id={t.downloadId}",
                Cookie(MemoryCache), AppInit.conf.Toloka.host);
    ...
    public TolokaController(IMemoryCache memoryCache, ITorrentRepository torrentRepository, HttpService httpService) :
        base(memoryCache)
    {
    ...
    {
        if (memoryCache.TryGetValue("cron:TolokaController:Cookie", out string cookie)) return cookie;

        return null;
    }

    private static async Task<bool> TakeLogin(IMemoryCache memoryCache)
    {
        try
        {
                        ...
                        if (response.Headers.TryGetValues("Set-Cookie", out var cook))
                        {
                            ...
                            foreach (var line in cook)
                            {
                                ...
                                if (line.Contains("toloka_sid="))
                                    toloka_sid = new Regex("toloka_sid=([^;]+)(;|$)").Match(line)
                                        .Groups[1].Value;

                                if (line.Contains("toloka_data="))
                                    toloka_data = new Regex("toloka_data=([^;]+)(;|$)").Match(line)
                                        .Groups[1].Value;
                            }

                            if (!string.IsNullOrWhiteSpace(toloka_sid) && !string.IsNullOrWhiteSpace(toloka_data))
                            {
                                memoryCache.Set("cron:TolokaController:Cookie",
                                    $"toloka_sid={toloka_sid}; toloka_ssl=1; toloka_data={toloka_data};",
