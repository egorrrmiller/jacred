...
public class TolokaController : BaseController
{
	...
	public async Task<string> UpdateTasksParse()
	{
		...
		{
			// Получаем html
			var html = await HttpClient.Get($"{AppInit.conf.Toloka.host}/f{cat}", timeoutSeconds: 10,
	...
	private async Task<bool> parsePage(string cat, int page)
	{
		...
		var html = await HttpClient.Get($"{AppInit.conf.Toloka.host}/f{cat}{(page == 0 ? "" : $"-{page * 45}")}?sort=8",
					...
					.Skip(1))
		{
			...
			if (!DateTime.TryParse(_createTime, out var createTime) || createTime == default)
			{
				continue;
			}
				...
				} else
				{
					...
					} else
					{
							...
							&& !string.IsNullOrWhiteSpace(g[5].Value))
						{
							...
							if (int.TryParse(g[5].Value, out var _yer))
							{
								relased = _yer;
							}
						} else
		...
		await _torrentRepository.AddOrUpdateAsync(torrents, async (t, db) =>
		{
			...
			var torrent = await HttpClient.Download($"{AppInit.conf.Toloka.host}/download.php?id={t.downloadId}",
	...
	public TolokaController(IMemoryCache memoryCache, ITorrentRepository torrentRepository) : base(memoryCache)
	{
